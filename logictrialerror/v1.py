...
requirement_task = Task(
    description=(
        "Analyze the requirements for an event categorized as '{event_type}' with approximately {guest_count} guests. "
        ...
        "Consider the specific needs of a {event_type} event - for example, a wedding might need different services than a corporate event. "
...
budget_task = Task(
    description=(
        "Allocate {budget} INR for {event_type} event with {guest_count} guests and these services: {services}. "
        "Provide a detailed budget breakdown for each service, ensuring the total matches the overall budget. "
        "Consider the typical costs for a {event_type} event in {location}. Allocate more budget to critical services "
...
venue_search_task = Task(
    description=(
        "Conduct a comprehensive venue search in {location} for the {event_type} event using the UniversalVenueServiceTool. "
        "Search for venues with these parameters: {venue_type} for {guest_count} guests within a budget of {service_budget} INR. "
...
vendor_search_task = Task(
    description=(
        "Based on the selected service type '{service_type}', conduct a comprehensive vendor search in {location} for the {event_type} event. "
            ...
            if 1 <= service_num <= len(approved_services):
                ...
                if selected_service['service'].lower() == "venue":
                    ...
                    venue_inputs = {
                        ...
                        "budget": selected_service['budget'],
...
def create_invitation(event_details):
    ...
    if 'guest_count' not in event_details and 'guest_count' in event_details:
        event_details['guest_count'] = event_details['guest_count']
    ...
    result = invitation_tool._run(
        ...
        host_name=event_details['host_name'],
        guest_count=event_details.get('guest_count') or event_details.get('guest_count', 30),
...
def get_event_details():
    ...
    details = {
        ...
        "event_type": input("Event Type (e.g., wedding, birthday party, corporate event): ").lower(),
        "event_date": input("Date (YYYY-MM-DD): "),
        "guest_count": int(input("Guest Count: ")),
    ...
    try:
        ...
        try:
            ...
            if service_names:
                ...
                event_type = details["event_type"].lower()
